...
export default function ProfileScreen() {
  ...
  const LANGUAGES: Array<{ code: 'fr' | 'en' | 'ar'; label: string; flag: string }> = [
  ...
  return (
    <View style={[s.container, { backgroundColor: C.bg }]}>
      ...
      {/* Security Modal */}
      <Modal visible={showSecurityModal} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setShowSecurityModal(false)}>
        <View style={[s.modal, { backgroundColor: C.bg }]}>
          <LinearGradient colors={[C.bg, C.bgDeep]} style={StyleSheet.absoluteFill} />
          <View style={[s.modalHandle, { backgroundColor: C.border2 }]} />
          <View style={s.modalHeader}>
